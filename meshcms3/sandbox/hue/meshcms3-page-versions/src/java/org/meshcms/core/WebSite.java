...
public class WebSite {
  ...
  /**
   * A prefix to be used for every draft file created in the repository.
   */
  public static final String DRAFT_PREFIX = "_draft_";
  ...
  protected SiteMap siteMapFull;
  protected SiteMap siteMapGuest;
  ...
  public void updateSiteMap(boolean force) {
    ...
    } else if (System.currentTimeMillis() - siteMapFull.getLastModified() >
               configuration.getUpdateIntervalMillis()) {
      new SiteMap(this).start();
      new DirectoryCleanerByAge(getFile(repositoryPath),
          configuration.getBackupLifeMillis()).start();
      new DirectoryCleanerByAge(getFile(generatedFilesPath),
          configuration.getBackupLifeMillis()).start();
    }
  }

  /**
   * Sets the <code>SiteMap</code> used for guests users that does not have
   * pages that are found on hidden paths. 
   */
  void setSiteMapGuest(SiteMap guestSiteMap) {
    this.siteMapGuest = guestSiteMap;
  }

  /**
   * NB: This method is deprecated - use getSiteMap(UserInfo) instead.
   ...
   * the site map. Since this object can be replaced with a new one at any
   * moment, a class that wants to use it should store it in a local variable
   * and use it for all the operation/method. The UserInfo is required as the
   * returned object is dependent on whether the user is a guest or not.
   */
  public SiteMap getSiteMap(UserInfo userInfo) {
  ...
  /**
   * Sets the full <code>SiteMap</code> that has all pages, including those
   * that exist on hidden paths.  
   */
  public void setSiteMapFull(SiteMap fullSiteMap) {
		this.siteMapFull = fullSiteMap;
	}

  /**
   * Returns the full <code>SiteMap</code> that has all pages, including those
   * that exist on hidden paths.
   */
  public SiteMap getSiteMapFull() {
		return siteMapFull;
	}
  ...
  /**
   * Stores an object into a file but extended to deal with saving as draft.
   */
  public boolean saveToFile(UserInfo user, Object saveThis, Path filePath,
      String charset, boolean saveAsDraft, boolean removeDraft) {
  	// If draft is not enabled - revert to original routine
  	if (! getConfiguration().isEnableSaveAsDraft()) {
  		return saveToFile(user, saveThis, filePath, charset);
  	} 
  	PageInfo pageInfo = siteMapFull.getPageInfo(filePath);
  	// Try to get pagePath from sitemap and fallback to stripping welcome file
  	Path pagePath = pageInfo != null ? pageInfo.getPath() : stripWelcomeFile(filePath);
  	SiteInfo siteInfo = this.getSiteInfo();
  	boolean isDraftPage = siteInfo.getDraft(pagePath);
  	boolean success = true;
    if (isDraftPage && removeDraft) {
    	// We're removing the draft - so unset it
    	siteInfo.setDraft(pagePath, false);
    	success = siteInfo.store();
    	if (!success) {
    		sc.log("Can't save draft status to non-draft");
    	}
    	// Check if existing page was 'created as draft' and delete
      File file = getFile(filePath);
    	if (file.isFile() && file.exists() && file.length() == 0) {
    		if (Utils.forceDelete(file)) {
      		...
      		dir.delete();
    			...
    			success = false;
	    		sc.log("Can't delete 0-length file: "+file.toString());
    		}
    	}
    } else if (saveAsDraft) {
    	// If it's a new draft - set flag
    	if(!isDraftPage) {
	    	siteInfo.setDraft(pagePath, true);
	    	success = siteInfo.store();
	    	if (!success) {
	    		sc.log("Can't save draft status to draft");
	    	}
    	}
      File file = getFile(filePath);
    	if (success) {
	    	// If the page is a new page - then create 0 length
	    	if(!file.exists()) {
	    		success = saveToFile(user, "", filePath, charset);
	    		if (success) {
	    			// Need to determine locally as page not in sitemap yet
	    			siteInfo.setHidden(pagePath, true);
	  	    	success = siteInfo.store();
	  	    	if (!success) {
	  	    		sc.log("Can't save hidden status to true");
	  	    	}
	    		}
	    	}
    	}
    	if (success) {
      	// Set the file path to save direct to tmp file in repository
	      String fileExt = getRepositoryFileExtension(file.getName());
	   		Path draftPath = repositoryPath.add(filePath).add(DRAFT_PREFIX +
	          user.getUsername() + "_" +
	          WebUtils.numericDateFormatter.format(new Date()) + fileExt);
	   		success = saveToFile(user, saveThis, draftPath, charset);
    	}
    } else {
    	// If we're moving from draft to real - unset draft status
    	if (isDraftPage && !saveAsDraft) {
	    	siteInfo.setDraft(pagePath, false);
	    	success = siteInfo.store();
	    	if (!success) {
	    		sc.log("Can't save draft status to non-draft");
    	...
    	// Check if existing page was 'created as draft' and unhide
    	if (success) {
	      File file = getFile(filePath);
	    	if (siteInfo.getHidden(pagePath) && file.isFile() && file.exists() && file.length() == 0) {
		    	siteInfo.setHidden(pagePath, false);
		    	success = siteInfo.store();
		    	if (!success) {
		    		sc.log("Can't save hidden status to non-hidden");
		    	}
	    	}
    	}
    	// Pass-on to correct save routine
    	success = saveToFile(user, saveThis, filePath, charset);
    }
    return success;
  }
  ...
  /**
   * Returns the draft file for a given file path.
   */
  public File getRepositoryDraftFile(Path filePath) {
    ...
    if (files!=null && files.length>0) {
		...
		if ( file==null || ( file != null && ! file.getName().startsWith(DRAFT_PREFIX))) {
      // Something's wrong - there's no latest file or it's not of type draft - try fix
			file = null;
      siteInfo.setDraft(filePath, false);
      try {
        siteInfo.store();
      } catch (Exception ex) { }
    }
  ...
  /**
   *  Returns the correct extension for a repository file or .bak if none.
   */
  private String getRepositoryFileExtension(String fileName) {
    int dot = fileName.lastIndexOf('.');
    return (dot == -1) ? ".bak" : fileName.substring(dot);
  }
  ...
  public Path getModulePath(String moduleName) {
    ...
    return (Path) siteMapFull.getModulesMap().get(moduleName);
  }
  ...
  /**
   * Strips the welcome file from a path just by looking at last path
   * element. This has limitations and getPathInMenu(...) in SiteMap
   * should be preferred instead unless the page is not yet in SiteMap
   * and this function can be used instead.
   */
  public Path stripWelcomeFile(Path pagePath) {
    return isWelcomeFileName(pagePath.getLastElement()) ? pagePath.getParent() : pagePath;
  }
