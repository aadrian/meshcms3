...
public final class HitFilter implements Filter {
  ...
  /**
   * Value of request atribute used to hold the overridden page content. 
   */
  public static final String PAGE_OVERRIDE = "meshcmspageoverride";
  ...
  public void doFilter(ServletRequest request, ServletResponse response,
    ...
    if (filterConfig != null && (request instanceof HttpServletRequest)) {
      ...
      if (webSite.getCMSPath() != null) {
        ...
        siteMap = webSite.getSiteMap(userInfo);

        /* Return 404 for hidden paths - if enabled - and for guests only */
        if (! isAdminPage && webSite.getConfiguration().isEnableHiddenPaths() && isGuest
        		&& siteInfo != null && siteInfo.isPathHidden(pagePath)) {
          httpRes.sendError(HttpServletResponse.SC_NOT_FOUND);
          return;
        }
        ...
        if (FileTypes.isPage(pagePath)) {
          ...
          /* if (isGuest && webSite.isInsideModules(pagePath) &&
        	...
        	// If logged-on user then check for page override - if this is enabled
        	if (!isGuest && webSite.getConfiguration().isEnableSaveAsDraft()) {
        		isDraft = siteInfo.getDraft(siteMap.getPathInMenu(pagePath));
        		if (isDraft) {
	      			// Try to add alternate page version to request
	      			File file = webSite.getRepositoryDraftFile(pagePath);
	      			FastPage pg = null;
	      			if (file != null) {
	              FastPageParser fpp = new FastPageParser();
	              try {
	                Reader reader = new BufferedReader(new FileReader(file));
	                pg = (FastPage) fpp.parse(reader);
	              } catch (Exception ex) { pg = null; }
	      			}
	      			if (pg != null) {
	      				request.setAttribute(PAGE_OVERRIDE, pg);
	      			}
        		}
        	}
