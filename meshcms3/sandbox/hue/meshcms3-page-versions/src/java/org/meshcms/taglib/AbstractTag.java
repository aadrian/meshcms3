...
public abstract class AbstractTag extends TagSupport implements RequestConstants {
  ...
  public int doStartTag() throws JspException {
    ...
    isEdit = HitFilter.ACTION_EDIT.equals(request.getParameter(HitFilter.ACTION_NAME)) &&
   	...
   	pageOverride = (FastPage)request.getAttribute(HitFilter.PAGE_OVERRIDE);
   	SiteInfo siteInfo = webSite.getSiteInfo(); 
    isDraft = siteInfo.getDraft(getSiteMapFromHelper().getPathInMenu(pagePath)); 
    isHidden = siteInfo.isPathHidden(pagePath);
  ...
  protected Page getPage() {
    ...
    if (p == null) {
    	// If we have an overridden page - then use this or use the default
      if (webSite.getConfiguration().isEnableSaveAsDraft()) {
      	p = (pageOverride != null) ? pageOverride : (Page) pageContext.getAttribute(PAGE, PageContext.REQUEST_SCOPE);
      } else {
  ...
  /**
   * A helper method to return the correct <code>SiteMap</code> for the user. 
   */
  protected SiteMap getSiteMapFromHelper() {
  	return webSite.getSiteMap(userInfo);
  }
}
