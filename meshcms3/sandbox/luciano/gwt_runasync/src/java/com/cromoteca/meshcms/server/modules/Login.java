...
public class Login extends ServerModule {
	...
	public void run() throws IOException, ServletException {
		...
		if (user == null) {
			...
			welcomeItem.getParameters()
					.put("text",
						"<p>"
						+ Context.getMessages().getString("loginWelcome")
	...
	public String getError() {
		return Context.getMessages().getString("loginError");
	}
				...
				try {
					...
					if (user == null) {
						return new FormSubmissionResult(form.getId(),
							Context.getConstants().getString("loginError"), true);
					} else {
						Context.setUser(user);

						return new FormSubmissionResult(form.getId(), null, false);
					}
