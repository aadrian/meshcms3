...
public class SiteConfigEditor extends FlowPanel implements FileManagerPanel {
	...
	private void buildLayout(final SiteConfiguration configuration,
		...
		searchField.getFieldWidget().addValueChangeHandler(new ValueChangeHandler<Boolean>() {
				public void onValueChange(ValueChangeEvent<Boolean> event) {
					overrideField.setRequired(searchField.getFieldWidget().getValue());
				}
			});
		...
		Field tidyField = new SelectionField(MeshCMS.CONSTANTS.configTidy()) {
				@Override
				public void populateOptions(ListBox listBox) {
					listBox.addItem(MeshCMS.CONSTANTS.configTidyNo(),
						Integer.toString(SiteConfiguration.TIDY_NO));
					listBox.addItem(MeshCMS.CONSTANTS.configTidyYes(),
						Integer.toString(SiteConfiguration.TIDY_YES));
					listBox.addItem(MeshCMS.CONSTANTS.configTidyAsk(),
						Integer.toString(SiteConfiguration.TIDY_ASK));
				}
			};

		systemPanel.add(tidyField);
		tidyField.setConnector(new FieldConnector() {
				public void storeValue(String value) {
					configuration.setTidy(Integer.parseInt(value));
				}

				public String retrieveValue() {
					return Integer.toString(configuration.getTidy());
				}
			});
		fields.add(tidyField);
