...
import com.cromoteca.meshcms.client.core.FileTypesWithIcons;
...
public class DetailedList extends FlexTable implements FileList, ClickHandler {
	private DateTimeFormat dateTimeFormat = DateTimeFormat
	...
	private void populateTable() {
		...
		for (FileInfo fileInfo : files) {
			...
			if (fileManager.isBrowser() || fileInfo.getPageInfo() == null) {
				...
				Path p = fileInfo.getPageInfo().isWelcome()
					? fileInfo.getPath().getParent() : fileInfo.getPath();
				setWidget(n, 1,
					new HTML("<a href=\"" + MeshCMS.CONTEXT_PATH + p.asAbsolute()
						+ "\" target=\"_top\">" + label + "</a>"));
			}

			setWidget(n, 2,
				new Label(fileInfo.isDirectory() ? "" : fileInfo.getLength(), false));
			setWidget(n, 3,
				new Label(dateTimeFormat.format(fileInfo.getLastModified()), false));
		}
	}
