...
public class ServerModule extends BackingBean {
	...
	public List<File> collectFiles(Path directory, String[] extensions,
		...
		if (dirFile != null) {
			...
			for (File file : dirFile.getChildren()) {
				if (file.isFile()) {
					if (check
		...
		if ("newestfirst".equals(order)) {
			Collections.sort(items,
				new Comparator<File>() {
					public int compare(File item1, File item2) {
						return Numbers.comparisonSign(item2.getLastModified(),
			...
			Collections.sort(items,
				new Comparator<File>() {
					public int compare(File item1, File item2) {
						return Numbers.comparisonSign(item1.getLastModified(),
			...
			if (zone != null) {
				...
				try {
					...
					BeanUtils.populate(getBean(),
						configuration.getModuleParameteres(zoneItem.getModuleName()));
