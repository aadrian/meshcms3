...
<form action="editmap2.jsp" method="post" name="sitemapform">
  <table class="meshcmseditor" border="0" cellspacing="0" cellpadding="0">
    <tr>
      ...
      <th><fmt:message key="mapSecured" /></th>
...
<%
  ...
  while (iter.hasNext()) {
    ...
    %><tr id="tr<%= code %>"><td style="padding-left: <%= (pagePath.getElementCount() + 1) * padding %>px;">
        ...
        String iCode = SiteInfo.getInternalCode(pagePath);
		...
		<%
		boolean internal = siteInfo.isInternal(pagePath);
		boolean parentInternal = WebUtils.isParentInternal(webSite, pagePath);
		
		if (userOk && !parentInternal) { %>
          <td><input type="radio" name="<%= iCode %>" id="<%= iCode %>" <%=internal?"checked='checked'":"" %> value="true"><img src="filemanager/images/lock.png" alt=""/><!-- <%=internal%> - <%=parentInternal%> -->&nbsp;<input type="radio" name="<%= iCode %>" id="<%= iCode %>" <%=internal?"":"checked='checked'" %> value="false"><img src="filemanager/images/lock_go.png" alt=""/></td>
        <% } else { %>
          <td style="text-align:center;"><%=parentInternal||internal?"<img src='filemanager/images/lock.png' alt=''/>":"<img src='filemanager/images/lock_go.png' alt=''/>" %></td>
        <% } %>
