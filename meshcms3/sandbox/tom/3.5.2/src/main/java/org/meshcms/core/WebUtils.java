...
public final class WebUtils {
  ...
  public static boolean isInternal(WebSite webSite, Path pagePath){
      if( webSite.getSiteInfo().isInternal(pagePath) ) return true;
      return isParentInternal(webSite, pagePath);
  }
  
  //Prüft ob eine der übergeordneten Seiten intern sind.
  public static boolean isParentInternal(WebSite webSite, Path pagePath) {
      if( Utils.isNullOrEmpty( pagePath.toString() ) ) return false;
      
      Path iterratorPath = pagePath.getParent();
      while( true ){
          if( iterratorPath.isRoot() ){
              break;
          }
          
          if( webSite.getSiteInfo().isInternal( iterratorPath ) ){
              return true;
          }
          
          iterratorPath = iterratorPath.getParent();
      }
      return false;
  }
}
