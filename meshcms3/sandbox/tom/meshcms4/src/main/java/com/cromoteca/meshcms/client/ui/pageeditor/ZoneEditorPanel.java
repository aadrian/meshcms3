...
public class ZoneEditorPanel extends AbsolutePanel {
	...
	public ZoneEditorPanel(FieldList fields, Zone zone, String title) {
		this.fields = fields;
		this.zone = zone;
		buildLayout(title);
	}
	...
	private void buildLayout(String title) {
		...
		if (title != null) {
			...
			titleLabel.setStylePrimaryName("mesh-field-caption");
		...
		if (Zone.isInheritable(zone.getName())) {
		...
		moduleList.add(new Button(MeshCMS.CONSTANTS.genericAdd(),
				new ClickHandler() {
	...
	private void addItem(final ZoneItem item, boolean initValues, boolean opened) {
		...
		for (final Parameter parameter : parameters) {
			if (!parameter.isSiteWide()) {
				Field field = createField(parameter, module.getName(),
						MeshCMS.CURRENT_DIR);
