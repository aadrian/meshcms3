...
public class Server {
	...
	static Server init(Path cmsPath) throws IOException {
		...
		s.rootFile = new File(sc.getRealPath("/"));
		...
		s.initModules();
		s.serverConfiguration = Context.loadServerConfiguration(new File(
					s.rootFile,
					serverConfigurationFilePath.toString()));
	...
	public Module getModule(String name) throws IOException {
		// TODO: remove static path /meshcms
		InputStream jsonStream = new FileInputStream(new File(rootFile,
	...
	private void initModules() throws IOException {
		...
		systemModules.add(getModule("canonicalurl"));
		...
		systemModules.add(getModule("mailform"));
