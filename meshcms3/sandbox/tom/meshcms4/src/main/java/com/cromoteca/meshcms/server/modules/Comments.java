...
public class Comments extends ServerModule {
	...
	private List<Comment> getComments() {
		...
		for (File commentFile : commentsDir.listFiles()) {
			Comment c = null;
			...
			try {
				if ("txt".equals(Strings.getExtension(commentFile.getName(), false))) {
					...
					if (c.getDate() == null) {
						c.setDate(new Date(commentFile.lastModified()));
					}
				} else {
	...
	public class CommentSubmitter implements FormSubmitHandler {
		public FormSubmissionResult submit() {
			...
			try {
				Context.storeToJSON(c, new File(commentsDir, commentFileName));
			...
			return new FormSubmissionResult(form.getId(),
				Context.getConstants().getString("commentsSubmitted"), false, null);
		}
	}
}
