...
import com.cromoteca.meshcms.server.core.Authorization;
...
public class ServicesImpl extends RemoteServiceServlet implements Services {
		...
		if (files != null && files.length > 0) {
			...
			for (File file : files) {
				...
				if (include) {
					...
					fileInfo.setLength(WebUtils.formatFileLength(file.length()));
	...
	public Outcome<Boolean> createDirectory(Path parentPath, String dirName)
		throws AuthorizationException {
		new Authorization().verified(parentPath);
	...
	public Outcome<Boolean> moveFiles(List<Path> files, Path newDir)
		throws AuthorizationException {
		Authorization authorization = new Authorization().verified(newDir);
	...
	public Outcome<Boolean> duplicateFile(Path filePath, String newName)
		throws AuthorizationException {
		new Authorization().verified(filePath);
	...
	public Outcome<Boolean> copyFiles(List<Path> files, Path newDir)
		throws AuthorizationException {
		new Authorization().verified(newDir);
	...
	public Outcome<Boolean> changeDates(List<Path> files, Date date)
		...
		for (Path path : files) {
			...
			oldDate.setTimeInMillis(file.lastModified());
	...
	public Outcome<Boolean> renameFile(Path filePath, String newName)
		throws AuthorizationException {
		new Authorization().verified(filePath.getParent());
...
//		}
	...
	public Outcome<Null> unzipFile(Path zipPath, String dirName)
		...
		new Authorization().verified(dir);
	...
	private Page fetchPage(Path pagePath, boolean searchDraft)
		...
		if (pageFile.isFile()) {
			InputStream inputStream = new FileInputStream(pageFile);
	...
	public Outcome<Null> savePage(Path pagePath, Page page, boolean asDraft)
		...
		try {
			...
			webSite.getFile(pagePath.getParent()).mkdirs();
	...
	public Outcome<Null> deleteDraft(Path pagePath)
		throws IOException, AuthorizationException {
		new Authorization().verified(pagePath);
	...
	public Outcome<Null> updatePersistentPageData(List<FileInfo> files)
		...
		for (FileInfo fileInfo : files) {
			verifier.verified(fileInfo.getPath());
		}
	...
	public Outcome<SitesConfiguration> getVirtualSites()
		throws AuthorizationException {
		new Authorization().verifiedAdmin();
	...
	public Outcome<Pair<SiteConfiguration, HashMap<String, String>>> getSiteConfigurationAndLocales()
		throws AuthorizationException {
		Authorization verifier = new Authorization().verifiedAdmin();
	...
	@ServiceMethod
	public Outcome<Map<String, String>> getModuleParameters()
		throws AuthorizationException {
		WebSite webSite = Context.getRequestContext().getWebSite();
		new Authorization().verified(webSite.getCMSPath(
				CMSDirectoryItem.EDITORS_DIR));

		Map<String, String> moduleParameters = webSite.getSiteConfiguration()
					.getModuleParameters();

		return new Outcome<Map<String, String>>(moduleParameters);
	}

	@ServiceMethod
	public Outcome<Null> saveModuleParameters(
		...
		SiteConfiguration configuration = webSite.getSiteConfiguration();
		configuration.setModuleParameters(moduleParameters);
		webSite.storeSiteConfiguration(configuration);
	...
	public Outcome<Pair<String, HashMap<String, String>>> getAvailableLocalesInUserLocale()
		throws AuthorizationException {
		Authorization verifier = new Authorization().verified();
	...
	@ServiceMethod
	public Outcome<String> getFile(Path filePath)
		throws IOException, AuthorizationException {
		new Authorization().verified(filePath);
		...
		return new Outcome<String>(text);
	}
	...
	@ServiceMethod
	public Outcome<Path> createPage(Path dirPath, String newPageAddress,
		String title) throws AuthorizationException, IOException {
		new Authorization().verified(dirPath);

		Outcome<Path> outcome = new Outcome<Path>();
		Path pageDirPath = dirPath.add(newPageAddress);
		WebSite webSite = Context.getRequestContext().getWebSite();
		File pageDir = webSite.getFile(pageDirPath);
		...
		if (pageDir.exists()) {
			...
			if (pageDir.isDirectory()) {
				Path pagePath = pageDirPath.add(Context.getServer().getWelcomeFiles()[0]);
				webSite.writeNewHTMLFile(pagePath, title);
				webSite.updateSiteMap(true);
				outcome.setValue(pageDirPath);
			} else {
				outcome.setMessage(Context.getMessages().getString("fmErrorNoNewDir")
							.replace("{0}", newPageAddress));
				outcome.setError(true);
			}
		}
	...
	public Outcome<ServerInfo> getServerInfo() throws AuthorizationException {
		...
		info.setUser(new Authorization().verified().getUser().getUserProfile());
	...
	public Outcome<Date> getFileDate(Path path) throws AuthorizationException {
		new Authorization().verified(path);
	...
	@ServiceMethod
	public Outcome<String> getLink(Path path, Path currentDir) {
		RequestContext rc = Context.getRequestContext();
		path = rc.getSiteMap().getPathInMenu(path);

		String link = path.equals(currentDir) ? "."
			: path.getRelativeTo(currentDir).toString();

		if (rc.getWebSite().isDirectory(path)) {
			link += '/';
		}

		return new Outcome<String>(link);
	}

	@ServiceMethod
	public Outcome<String> getHTMLPreview(String html, Path currentDir)
		throws AuthorizationException {
		new Authorization().verified();
		html = WebUtils.fixLinks(html, Context.getContextPath(), currentDir,
				Context.MESHCMS_PATH.add("resources"));

		return new Outcome<String>(html);
	}
}
