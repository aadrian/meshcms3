...
public class ModuleConfigEditor extends FlowPanel implements FileManagerPanel {
	...
	public ModuleConfigEditor() {
		new AuthorizableServerCall<Map<String, String>>() {
				@Override
				public void callServer() {
					MeshCMS.SERVER.getModuleParameters(getAsyncCallback());
				}
	...
	private void buildLayout(final Map<String, String> moduleParameters) {
					...
					if (field != null) {
						...
						FieldConnector connector = new FieldConnector() {
								public void storeValue(String value) {
									moduleParameters.put(module.getName() + ':' + param.getName(),
										value);
								}

								public String retrieveValue() {
									return moduleParameters.put(module.getName() + ':'
					...
					public void onClick(ClickEvent event) {
						if (fields.verifyAll()) {
							...
							new AuthorizableServerCall<Null>() {
									@Override
									public void callServer() {
										MeshCMS.SERVER.saveModuleParameters(moduleParameters,
											getAsyncCallback());
									}
		...
		fields.addActionButton(saveButton);
		fields.verifyAll();
	}
