...
public class MultiSiteManager {
	...
	public File getRootFile(VirtualWebSite site) {
		return site.isExternal() ? new File(site.getRoot())
	...
	public void initDomainMap() throws IOException {
		...
		for (VirtualWebSite site : configuration.getWebSites()) {
			if (configuration.isUseDirsAsDomains()) {
				...
				if (webSite == null) {
					if (site.isExternal()) {
						webSite = new WebSite(new File(site.getRoot()), null,
								site.isCMSEnabled());
					} else {
		...
		if (!parseDomains(newMap, mainWebSite, configuration.getMainWebSiteDomains())) {
