...
public final class HitFilter implements Filter{
    ...
    /**
     * Name of the request parameter that is used to specify some actions. Currently only {@link #ACTION_EDIT} is used as value. This parameter is read by custom JSP tags.
     */
    public static final String ACTION_NAME = "meshcmsaction";

    /**
     * Value of {@link #ACTION_NAME} used to indicate that the current page must be edited.
     */
    public static final String ACTION_EDIT = "edit";

    public static final String ROOT_WEBSITE = "meshcmsrootsite";

    public static final String INTERNAL_REDIRECT = "meshcmsredirect";
    ...
    public void doFilter( ServletRequest request, ServletResponse response, FilterChain chain ) throws IOException, ServletException{
        if( filterConfig != null && ( request instanceof HttpServletRequest ) ){
            ...
            if( webSite instanceof VirtualWebSite ){
                ...
                if( !pagePath.isContainedIn( webSite.getAdminPath() ) && uri.endsWith( ".jsp" ) && !WebUtils.verifyJSP( webSite, pagePath ) ){
                    httpRes.sendError( HttpServletResponse.SC_FORBIDDEN, "Execution of this page is not allowed" );
                    return;
                }
            }
            ...
            if( webSite.isDirectory( pagePath ) ){
                ...
                if( conf == null || conf.isAlwaysDenyDirectoryListings() ){
                    if( wPath != null ){
                        ...
                        httpRes.sendError( HttpServletResponse.SC_FORBIDDEN, "Directory listing denied" );
                    }
            ...
            if( webSite.getCMSPath() != null ){
                if( pagePath.isContainedIn( webSite.getVirtualSitesPath() ) || pagePath.isContainedIn( webSite.getPrivatePath() ) ){
                    httpRes.sendError( HttpServletResponse.SC_NOT_FOUND );
                    return;
                }
                ...
                if( webSite.getConfiguration().isSearchMovedPages() && !( isAdminPage || webSite.getFile( pagePath ).exists() ) ){
                ...
                // Prüfe ob die aktuelle Seite intern ist
                boolean internal = webSite.getSiteInfo().isInternal( pagePath.getParent() );

                // Wenn nicht, prüfe ob übergeordnete Seiten intern sind.
                if( !internal ){
                    internal = WebUtils.isParentInternal( webSite, pagePath );
                }

                // See if should redirect to one of the available languages
                if( isGuest && webSite.getConfiguration().isRedirectRoot() && siteMap.getPathInMenu( pagePath ).isRoot() ){
                ...
                if( isGuest && internal ){
                    ...
                    if( httpReq.getQueryString() != null ) url += "?" + httpReq.getQueryString();
                    session.setAttribute( INTERNAL_REDIRECT, url );
                    
                    httpRes.sendRedirect( httpReq.getContextPath() + "/meshcms/admin/login.jsp" );
                    return;
                }
                ...
                if( FileTypes.isPage( pagePath.getLastElement() ) ){
                    ...
                    /*
                     * if (isGuest && webSite.isInsideModules(pagePath) && pagePath.getLastElement().equalsIgnoreCase(SiteMap.MODULE_INCLUDE_FILE)) {
                     * httpRes.sendError(HttpServletResponse.SC_NOT_FOUND); return; }
                     */

                    // disallow access to guests if required by configuration
                    if( isGuest && webSite.getConfiguration().isPasswordProtected() && !pagePath.isChildOf( webSite.getAdminPath() ) ){
                        httpRes.sendError( HttpServletResponse.SC_FORBIDDEN, "You don't have enough privileges" );
                        return;
                    }
                    ...
                    if( themePath != null ){ // there is a theme for this page
                        if( themePath.isContainedIn( webSite.getCustomThemesPath() ) && !WebUtils.verifyJSP( webSite, themePath.add( "main.jsp" ) ) ){
                            if( isAdminPage ){
                                webSite.setLastAdminThemeBlock( System.currentTimeMillis() );
                            }

                            httpRes.sendError( HttpServletResponse.SC_FORBIDDEN, "Current theme is not allowed" );
                            return;
                        }
                        ...
                        if( !isAdminPage || themeParameter != null ){
                            request.setAttribute( THEME_FILE_ATTRIBUTE, "/" + webSite.getServedPath( themePath ) + "/" + SiteMap.THEME_DECORATOR );
                        }
                    }

                    /*
                     * Since a real page has been requested, disable hotlinking prevention for this session if (session != null && webSite.getConfiguration().isPreventHotlinking()
                     * && session.getAttribute(HOTLINKING_ALLOWED) == null && !isAdminPage) { session.setAttribute(HOTLINKING_ALLOWED, HOTLINKING_ALLOWED); }
                     */
                }

                if( webSite.getConfiguration().isPreventHotlinking() && FileTypes.isPreventHotlinking( pagePath.getLastElement() ) /*
                                                                                                                                    * && (session == null ||
                                                                                                                                    * session.getAttribute(HOTLINKING_ALLOWED) ==
                                                                                                                                    * null)
                                                                                                                                    */){
                        ...
                        try{
                            ...
                            if( domain != null ){
                                ...
                                if( referrer == null || referrer.indexOf( domain ) < 0 ){
                                    httpRes.sendRedirect( httpReq.getContextPath() + "/" + webSite.getAdminPath() + "/hotlinking.jsp?path=" + pagePath );
                                    return;
                                }
                            }
                        }
                ...
                if( pageInfo != null ){ // this page is contained in the site map
                    ...
                    if( isAdminPage && pagePath.getLastElement().endsWith( ".js" ) && !isGuest && userInfo.canDo( UserInfo.CAN_BROWSE_FILES ) ){
                        String script = null;
            ...
            httpRes.setContentType( mimeType + "; charset=" + Utils.noNull( pageCharset, Utils.SYSTEM_CHARSET ) );

            try{
                // Cache management
                if( isGuest && pageInfo != null && httpReq.getMethod().equalsIgnoreCase( "get" ) && Utils.isNullOrEmpty( httpReq.getQueryString() )
                    ...
                    if( cacheType == Configuration.IN_MEMORY_CACHE || cacheType == Configuration.MIXED_CACHE ){
                    ...
                    if( cacheType == Configuration.ON_DISK_CACHE || ( in == null && cacheType == Configuration.MIXED_CACHE ) ){
                    ...
                    if( cacheType != Configuration.NO_CACHE ){
                        ...
                        /*
                         * If WebUtils.setBlockCache has not been called while creating the page, it can be cached
                         */
                        if( !WebUtils.isCacheBlocked( httpReq ) ){
                            ...
                            if( cacheType == Configuration.ON_DISK_CACHE || cacheType == Configuration.MIXED_CACHE ){
                                File cacheFile = webSite.getRepositoryFile( siteMap.getServedPath( pagePath ), CACHE_FILE_NAME );
                ...
                sc.log( "--------\n\nIMPORTANT: an exception has been caught while serving " + httpReq.getRequestURI(), ex );
    ...
    private static void redirect( HttpServletRequest httpReq, HttpServletResponse httpRes, Path redirPath ) throws IOException{
        blockRemoteCaching( httpRes );
    ...
    public static WebSite getRootSite( ServletContext sc, boolean alwaysCreate ){
        ...
        if( rootSite == null || alwaysCreate ){
            ...
            rootSite = multisite ? MainWebSite.create( sc, WebUtils.getWelcomeFiles( sc ), rootFile, Path.ROOT, cmsPath ) : WebSite.create( sc, WebUtils.getWelcomeFiles( sc ),
                    rootFile, Path.ROOT, cmsPath );
