...
<%
  ...
  boolean isEnabledSaveAsDraft = configuration.isEnableSaveAsDraft();
  boolean isEnabledHiddenPaths = configuration.isEnableHiddenPaths();
%>
...
<% if (isEnabledSaveAsDraft) { %>
      <th><fmt:message key="mapDraft" /></th>
<% } %>
<% if (isEnabledHiddenPaths) { %>
      <th><fmt:message key="mapHidden" /></th>
<% } %>
  ...
  while (iter.hasNext()) {
    ...
    %><tr id="tr<%= code %>"><td style="padding-left: <%= (pagePath.getElementCount() + 1) * padding %>px;">
        ...
        if (userInfo.canWrite(webSite, pagePath)) {
          ...
          String hsCode = siteInfo.getHideSubmenuCode(pagePath);
          String drCode = siteInfo.getDraftCode(pagePath);
          String hiCode = siteInfo.getHiddenCode(pagePath);

          boolean draft = false;
          if (isEnabledSaveAsDraft) {
  	        draft = siteInfo.getDraft(pagePath);
    	      if (draft && webSite.getRepositoryDraftFile(siteMap.getServedPath(pagePath)) != null) { %>
              <td align="center"><img src="filemanager/images/button_yes.gif" alt=""
                  style='vertical-align:middle;' title="<fmt:message key="mapIsDraft" />" /></td>
<%       	  } else { %>
              <td>&nbsp;</td>
<% 	        }
          }
          
          boolean hidden = siteInfo.getHidden(pagePath);
          if (isEnabledHiddenPaths) {
	          if(draft && hidden && pageInfo.getBytes()==0) { %>
	            <td align="center"><img src="filemanager/images/button_yes.gif" alt=""
	                style='vertical-align:middle;' title="<fmt:message key="mapIsHiddenCreatedAsDraft" />" />
	              <input type="hidden" name="<%= hiCode %>" value="<%= hidden? "true" : "false" %>" />
	            </td>
<%	        } else { %>
	            <td align="center"><input type="checkbox" name="<%= hiCode %>" <%= hidden? "checked=\"checked\"" : "" %> value="true" />
	            </td>
<%	        }
          } %>
            <td align="center">
<%        // If we've got features disabled then include as hidden fields
          if (!isEnabledSaveAsDraft) { %>
              <input type="hidden" name="<%= drCode %>" value="<%= draft? "true" : "false" %>" />
<%        }
          if (!isEnabledHiddenPaths) { %>
              <input type="hidden" name="<%= hiCode %>" value="<%= hidden? "true" : "false" %>" />
<%        } %>
