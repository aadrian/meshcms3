...
public class FolderXTree extends DirectoryParser implements Finals {
  ...
  public FolderXTree(WebSite webSite, UserInfo userInfo, Writer writer,
    ...
    setRecursive(true);
    setSorted(true);
    setInitialDir(webSite.getFile(webSite.getRootPath()));
    setDaemon(true);
  ...
  protected boolean processDirectory(File file, Path path) {
    if (path.equals(webSite.getCMSPath().add(REPOSITORY_SUBDIRECTORY))) {
      return false;
    }
    ...
    try {
      ...
      if (webSite.isSystem(path, true) ||
          path.isContainedIn(webSite.getCMSPath().add(GENERATED_FILES_SUBDIRECTORY))) {
        include = false;
      } else if (path.isContainedIn(webSite.getCMSPath())) {
        imgType = "themes";
      }
