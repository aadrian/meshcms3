...
public final class Breadcrumbs extends AbstractTag {
  ...
  public void writeTag() throws IOException {
    PageInfo[] breadcrumbs = webApp.getSiteMap().getBreadcrumbs(pagePath);
    String[] outs;

    if (mode != null && mode.equals(MODE_LINKS)) {
      outs = webApp.getLinkList(breadcrumbs, request.getContextPath(), target,
                                style);
    } else {
      outs = webApp.getTitles(breadcrumbs);
    }
    ...
    if (Utils.isTrue(current)) {
      ...
      outs[last] = (pageInfo == null) ? getPage().getTitle() :
          webApp.getSiteInfo().getPageTitle(pageInfo);
    }
