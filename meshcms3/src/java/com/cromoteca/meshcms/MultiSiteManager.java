...
public class MultiSiteManager implements Finals, Serializable {
  ...
  public int initDomainsMap() {
    ...
    File[] dirs = mainWebSite.getFile
        (mainWebSite.getCMSPath().add(OTHER_SITES_SUBDIRECTORY)).listFiles();
  ...
  public static MultiSiteManager load(MainWebSite mainWebSite) {
    ...
    try {
      m = (MultiSiteManager) mainWebSite.loadFromXML
          (mainWebSite.getCMSPath().add(SITES_FILE));
    } catch (Exception ex) {}
  ...
  public boolean store(WebSite webSite) {
    return webSite.storeToXML(this, webSite.getCMSPath().add(SITES_FILE));
  }
}
