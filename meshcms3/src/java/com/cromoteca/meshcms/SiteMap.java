...
public class SiteMap extends DirectoryParser implements Finals {
  ...
  protected boolean processDirectory(File file, Path path) {
    if (!webSite.isSystem(path, false)) {
      Path wPath = findCurrentWelcome(path);
  ...
  public Path findCurrentWelcome(Path dirPath) {
    ...
    if (dirFile.isDirectory()) {
      String[] welcomeFiles = webSite.getWelcomeFileNames();
  ...
  public SortedMap getThemesMap() {
    if (themesMap == null) {
      ...
      addDirItemsToMap(themesMap,
          webSite.getAdminPath().add(THEMES_SUBDIRECTORY), THEME_DECORATOR);
      addDirItemsToMap(themesMap,
          webSite.getCMSPath().add(THEMES_SUBDIRECTORY), THEME_DECORATOR);
  ...
  public SortedMap getModulesMap() {
    if (modulesMap == null) {
      ...
      addDirItemsToMap(modulesMap,
          webSite.getAdminPath().add(MODULES_SUBDIRECTORY), MODULE_INCLUDE_FILE);
      addDirItemsToMap(modulesMap,
          webSite.getCMSPath().add(MODULES_SUBDIRECTORY), MODULE_INCLUDE_FILE);
