...
public class UserInfo implements Serializable {
  ...
  public boolean load(WebSite webSite, String username, String password) {
    ...
    File userFile = getUserFile(webSite, username);
    ...
    try {
      ...
      webSite.log("Can't load user file for " + username, ex);
      return false;
    } catch (Exception ex) {
      return false;
    } finally {
      if (fis != null) {
        try {
          ...
          webSite.log("Can't close user file for " + username, ex);
        }
      }
    }
  ...
  public boolean save(WebSite webSite) {
    File userFile = getUserFile(webSite, getUsername());
  ...
  private File getUserFile(WebSite webSite, String username) {
    return new File(webSite.getUsersDirectory(), username + ".txt");
  }
  ...
  public boolean canWrite(WebSite webSite, Path filePath) {
    if (filePath == null || !canDo(CAN_EDIT_PAGES) ||
        filePath.isContainedIn(webSite.getAdminPath())) {
      return false;
    }
