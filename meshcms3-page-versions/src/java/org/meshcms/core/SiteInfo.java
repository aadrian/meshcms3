...
public class SiteInfo {
  ...
  /**
   * Prefix of the draft codes.
   */
  public static final String DRAFT = "draft";

  /**
   * Prefix of the hidden codes.
   */
  ...
  public String getPageTitle(PageInfo pageInfo) {
    ...
    String customTitle = getPageTitle(pagePath);

    if (Utils.isNullOrEmpty(customTitle)) {
      customTitle = pageInfo.getTitle();
    }

    if (webSite.getConfiguration().isEnableHiddenPaths() && this.isPathHidden(pagePath)) {
  ...
  /**
   * Returns the draft setting for the given path (false if not available).
   */
  public boolean getDraft(Path pagePath) {
    return Utils.isTrue(data.getProperty(getDraftCode(pagePath)));
  }

  /**
   * Returns the draft setting as a string for the given path. An empty string
   * is returned if the draft setting is false.
   */
  public String getDraftAsString(Path pagePath) {
    boolean draft = getDraft(pagePath);
    return draft == false ? "" : Boolean.toString(draft);
  }

  /**
   * Sets the draft setting for the given path.
   */
  public void setDraft(Path pagePath, String draft) {
    setDraft(pagePath, Utils.isTrue(draft));
  }

  /**
   * Sets the draft setting for the given path. If the draft is false, it is removed
   * since false is the default.
   */
  public void setDraft(Path pagePath, boolean draft) {
    setDraft(getDraftCode(pagePath), draft);
  }

  private void setDraft(String pageCode, boolean draft) {
    if (draft == false) {
      data.remove(pageCode);
    } else {
      data.setProperty(pageCode, Boolean.toString(draft));
    }
  }
  
  /**
   * Returns the hidden setting for the given path (false if not available).
   */
  public boolean getHidden(Path pagePath) {
    return Utils.isTrue(data.getProperty(getHiddenCode(pagePath)));
  }

  /**
   * Returns the hidden setting as a string for the given path. An empty string
   * is returned if the hidden setting is false.
   */
  public String getHiddenAsString(Path pagePath) {
    boolean hidden = getHidden(pagePath);
    return hidden == false ? "" : Boolean.toString(hidden);
  }

  /**
   * Sets the hidden setting for the given path.
   */
  public void setHidden(Path pagePath, String hidden) {
    setHidden(pagePath, Utils.isTrue(hidden));
  }

  /**
   * Sets the hidden setting for the given path. If the hidden is false, it is removed
   * since false is the default.
   */
  public void setHidden(Path pagePath, boolean hidden) {
    setHidden(getHiddenCode(pagePath), hidden);
  }

  private void setHidden(String pageCode, boolean hidden) {
    if (hidden == false) {
      data.remove(pageCode);
    } else {
      data.setProperty(pageCode, Boolean.toString(hidden));
    }
  }
  ...
  /**
   * Returns the code for the draft field of the given path. This code is
   * used in the HTML configuration form and in the config file.
   */
  public static String getDraftCode(Path pagePath) {
    return DRAFT + WebUtils.getMenuCode(pagePath);
  }
  
  /**
   * Returns the code for the hidden field of the given path. This code is
   * used in the HTML configuration form and in the config file.
   */
  public static String getHiddenCode(Path pagePath) {
    return HIDDEN + WebUtils.getMenuCode(pagePath);
  }
  ...
  public boolean setValue(String fieldName, String value) {
    ...
    if (fieldName.startsWith(HIDDEN)) {
      boolean b = Utils.isTrue(value);
  ...
  protected Path getThemePath(Path pagePath) {
    do {
      ...
      if (!Utils.isNullOrEmpty(theme)) {
        ...
        Path themePath = (Path) webSite.getSiteMapFull().getThemesMap().get(theme);
  ...
  public Path getThemeRoot(Path pagePath) {
    do {
      ...
      if (!Utils.isNullOrEmpty(theme)) {
        if (webSite.getSiteMapFull().getThemesMap().get(theme) != null) {
          return pagePath;
        }
      }
  ...
  /**
   * Returns a boolean that determines whether the passed-in path
   * is directly hidden or is contained within a hidden path.
   */
  public boolean isPathHidden(Path pagePath) {
  	if (webSite.getConfiguration().isEnableHiddenPaths()) {
	    do {
	      if (getHidden(pagePath)) {
	        return true;
	      }
  ...
  /**
	...
	protected boolean cleanupSiteInfo() {
		...
		List pagesList = webSite.getSiteMapFull().getPagesList();
		if (pagesList != null) {
			...
			while (iter.hasNext()) {
				...
				newSiteInfo.setDraft(pagePath, getDraft(pagePath));
				newSiteInfo.setHidden(pagePath, getHidden(pagePath));
			}
