...
public class PageBody extends AbstractTag {
  ...
  public void writeEditTag() throws IOException {
    ...
    if (webSite.getConfiguration().isEnableSaveAsDraft()) {
	    w.write("<div class='meshcmsfield'><input type='checkbox' "+ (isDraft?"checked='checked'":"") +" id='draft' name='draft' \n");
	    w.write(" onclick=\"javascript:if(!this.checked && !confirm('" + bundle.getString("editorDraftSaveConfirm") + "')){this.checked=true;}\" />\n");
	    w.write(" <label for='draft'>" + bundle.getString("editorDraftSave") + "</label></div>\n");
		
	    if (isDraft) {
		    w.write("<div class='meshcmsfield'><input type='checkbox' id='rmdraft' name='rmdraft' \n");
		    w.write(" onclick=\"javascript:if(this.checked && !confirm('" + bundle.getString("editorDraftRemoveConfirm") + "')){this.checked=false;};draft.checked=!this.checked;draft.parentNode.style.display=this.checked?'none':'';\" />\n");
		    w.write(" <label for='rmdraft'>" + bundle.getString("editorDraftRemove") + "</label></div>\n");
	    }
    }
