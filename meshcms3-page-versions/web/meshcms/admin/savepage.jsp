...
<%
  ...
  boolean saveAsDraft = Utils.isTrue(request.getParameter("draft"));
  boolean removeDraft = Utils.isTrue(request.getParameter("rmdraft"));
  Path filePath = null;
  String title = null;
  String charset = null;
  ...
  if (Utils.isNullOrEmpty(fullSrc)) {
    ...
    PageInfo pageInfo = webSite.getSiteMapFull().getPageInfo(filePath);
...
<%
  if (webSite.saveToFile(userInfo, fullSrc, filePath, charset, saveAsDraft, removeDraft)) {
...
%>
  ...
  <% if (removeDraft && !webSite.getFile(filePath).exists() ) { %>
  <p><fmt:message key="saveDeleted" /></p>
  <% } else { %>
