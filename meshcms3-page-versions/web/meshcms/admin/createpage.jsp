...
<% String title = Utils.noNull(request.getParameter("title")).trim();
...
<% if (webSite.getConfiguration().isEnableSaveAsDraft()) { %>
      <tr><td align='center'>
        <input type='checkbox'<%=Utils.isTrue(request.getParameter("draft")) ? "checked='checked'" : ""%> name='draft' value='true' />
        <fmt:message key="newpageDraft" />
      </td></tr>
<% } %>
      <tr><th align='center'>
...
<% } else { 
  ...
  boolean createAsDraft = Utils.isTrue(request.getParameter("draft"));
  ...
  if (fileName == null) {
    ...
    if (webSite.saveToFile(userInfo, text, path, null, createAsDraft, false)) {
