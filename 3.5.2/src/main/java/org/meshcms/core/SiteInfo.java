...
public class SiteInfo {
  ...
  public boolean isInternal(Path pagePath) {
      return "true".equals( data.getProperty(getInternalCode(pagePath) ) );
  }
  
  public void setInternal( Path pagePath, String internal ){
      if (Utils.isNullOrEmpty(internal) || "false".equals( internal )) {
          data.remove(getThemeCode(pagePath));
        } else {
          data.setProperty(getThemeCode(pagePath), internal);
        } 
  }
  ...
  public static String getInternalCode(Path pagePath) {
      return INTERNAL + WebUtils.getMenuCode(pagePath);
    }
  ...
  public boolean setValue(String fieldName, String value) {
    ...
    if (fieldName.startsWith( INTERNAL )) {
        boolean b = Utils.isTrue(value);
